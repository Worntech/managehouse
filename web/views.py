...
def manual_control(request):
    try:
        # Fetch sensor state from the ESP8266
        response = requests.get(f"{ESP8266_IP}/get_data", timeout=5)
        response.raise_for_status()  # Raise exception for HTTP errors
        data = response.json()  # Parse JSON response
        state = data.get("state", "No data")
    except requests.exceptions.RequestException as e:
        state = f"Error: {str(e)}"
    return render(request, 'web/manual_control.html', {"state": state})
...
def led_on(request):
    try:
        response = requests.get(f"{ESP8266_IP}/web/on", timeout=5)  # Request to ESP8266
        response.raise_for_status()  # Raise HTTPError for bad responses (4xx and 5xx)
        # return JsonResponse({"status": "LED On"})  # Return success response
        return redirect('led_control')
    except requests.exceptions.RequestException as e:
        return JsonResponse({"status": f"Error: {str(e)}"}, status=500)  # Handle errors

def led_off(request):
    try:
        response = requests.get(f"{ESP8266_IP}/web/off", timeout=5)  # Request to ESP8266
        response.raise_for_status()  # Raise HTTPError for bad responses (4xx and 5xx)
        # return JsonResponse({"status": "LED Off"})  # Return success response
        return redirect('led_control')
    except requests.exceptions.RequestException as e:
        return JsonResponse({"status": f"Error: {str(e)}"}, status=500)  # Handle errors
...
def display_data(request):
    try:
        # Fetch sensor data from the ESP8266
        response = requests.get(f"{ESP8266_IP}/get_data", timeout=5)
        response.raise_for_status()  # Raise exception for HTTP errors
        data = response.json()  # Parse JSON response
        sensor_value = data.get("sensor_value", "No data")
    except requests.exceptions.RequestException as e:
        sensor_value = f"Error: {str(e)}"

    return render(request, "web/display_data.html", {"sensor_value": sensor_value})


def highdoorone(request):
    try:
...
def lowdoorone(request):
    try:
        response = requests.get(f"{ESP8266_IP}/lowdoorone", timeout=5)
        response.raise_for_status()
        # return JsonResponse({"status": "success", "state": "LOW"})
        return redirect('manual_control')
    except requests.exceptions.RequestException as e:
        return JsonResponse({"status": "error", "message": str(e)})
    
def highdoortwo(request):
    try:
...
def highdoorthree(request):
    try:
        response = requests.get(f"{ESP8266_IP}/highdoorthree", timeout=5)
        response.raise_for_status()
        # return JsonResponse({"status": "success", "state": "LOW"})
        return redirect('manual_control')
    except requests.exceptions.RequestException as e:
...
def highdoorfour(request):
    try:
        response = requests.get(f"{ESP8266_IP}/highdoorfour", timeout=5)
        response.raise_for_status()
        # return JsonResponse({"status": "success", "state": "LOW"})
        return redirect('manual_control')
    except requests.exceptions.RequestException as e:
...
def highdoorfive(request):
    try:
        response = requests.get(f"{ESP8266_IP}/highdoorfive", timeout=5)
        response.raise_for_status()
        # return JsonResponse({"status": "success", "state": "LOW"})
        return redirect('manual_control')
    except requests.exceptions.RequestException as e:
